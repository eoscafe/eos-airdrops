...
test("Must have exactly one token entry", async () => {
    // Fetch origin/master first (needed for GitHub Actions shallow clone)
    Bun.spawnSync(["git", "fetch", "origin", "master"]);
    const result = Bun.spawnSync(["git", "show", "origin/master:tokens.json"]);
    if (result.exitCode !== 0) {
        throw new Error("Failed to fetch tokens.json from origin/master");
    }
    const masterTokens = JSON.parse(result.stdout.toString());
    const masterTokenKeys = new Set(masterTokens.map((t: (typeof tokens)[0]) => `${t.chain},${t.symbol},${t.account}`));
    const addedTokens = tokens.filter((t) => !masterTokenKeys.has(`${t.chain},${t.symbol},${t.account}`));

    expect(addedTokens.length).toBeLessThanOrEqual(1);
});
